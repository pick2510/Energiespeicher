...
\begin{figure}[h!]
\begin{center}
\includegraphics[scale=0.2]{images/heatconductionin.jpg}
...
\begin{figure}[h!]
\begin{center}
\includegraphics[scale=0.2]{images/threedimensionalphysicalmodel.jpg}
...
\begin{figure}[h!]
\begin{center}
\includegraphics[scale=0.2]{images/configurationofthecarbonnanofibers.jpg}
...
Gleich vorweg, im Gegensatz zu sensiblen oder latenten Wärmespeichern, die bereits kommerziell erhältlich sind, sind thermochemische Speicher bislang nur Gegenstand der Forschung \cite{gegenstandderforschung}. So oder so soll in diesem Kapitel näher darauf eingegangen werden, da dieser Art von Speicher auch seine Vorzüge hat nachfolgend ein paar Gründe.
\begin{itemize}
\item Speicherung grosser Wärmemengen, da eine hohe Energiedichte 
\item Grosses Spektrum von verfügbaren Temperaturen
\item Specherung über lange Zeiträme ohne grosse Verluste
\end{itemize}

\subsubsection{Arbeitsprinzip}
Bei der thermochemischen Speicherung werden chemische Reaktionen genützt um Wärme in einem Material zu speichern. Das Arbeitsprinzip dieser Speicher ist in Abbildung \ref{fig:ladenspeichernentladens} ersichtlich. Wenn dem Material Wärme zugeführt wird, wird es in zwei Komponenten zerlegt. Diese beiden Komponenten können für lange Zeit ohne Energieverlust gespeichert werden. Werden diese beiden wieder zusammengebracht, so findet die Umkehrreaktion statt und die Wärme wird wieder frei. 
...
Wird dies Technisch mittels Sorption umgesetzt, wo sich unterteilen lässt in Adsorbtion und Absorption. Diese können jeweils durch ein offenes oder geschlossenen Systemen umgesetzt werden.


\subsubsection{Stand der Technik der Adsorption}
Die Abbildung \ref{fig:adsorption} zeigt der Prozess einer geschlossenen Adsorption.
...
Technisch würde ein System aus einem Porösen Festkörper. 

Beim Laden des Speichers, siehe Abbildung \ref{fig:geschlossenerladen} wird Wasser unter Wärmezufuhr aus Speichermaterial desorbiert. Die Kondensation des Wasserdampf auf der passiert hier auf der linken Seite. Dabei muss gewährleistet sein, dass $p_{v/k}$ < $p_R$ < 1bar ist. 
...
Bei der Entladung des Speichers wird das Wasser unter Vakuum verdampft. Es passiert die Adsorption von Wasserdampfmolekülen am Speichermatierial und es wird Wärme abgegeben. Dabei muss gewährleistet sein, dass 1bar > $p_{v/k}$ > $p_R$ ist. 


\begin{figure}[h!]
\begin{center}
\includegraphics[scale=0.6]{images/geschlossenerentladen.jpg}
\caption{Offenes Adsorptionssystem mit festem Adsorbens}
\label{fig:geschlossenerentladen}
\end{center}
\end{figure}


Eine andere Möglichkeit ist die Adsorption mittels offenes System, welches in der Abbildung \ref{fig:offenessystemadsorption} dargestellt ist. Diese beinahtlen ein festen Adsorbentien (z.B. Zeolithen oder Silikagelen).  Bei der offenen Adsorptionssystem transportiert der Luftstrom die Wärme und den Wasserdampf in und aus einer Adsorbensschüttung. Somit werden die Lufttemperatur und gleichzeitig der Wasserdampfpartialdruck des Luftstroms durch den Sorptionsprozess beeinflusst. Die umgestzen Stoff- und Wärmemengen sind in der \ref{fig:offenessystemadsorption} angedeutet.


\begin{figure}[h!]
\begin{center}
\includegraphics[scale=0.6]{images/offenessystemadsorption.jpg}
...
\begin{figure}[h!]
\begin{center}
\includegraphics[scale=0.6]{images/offenessystemabsorption.jpg}
\caption{Entladen}
...
Zum Bereich mit der Absorption des geschlossenen System konnten keine Informationen gefunden werden. Dies wird jedoch in einer Wärmepumpe eingesetzt, ist aber kein Speicher.
